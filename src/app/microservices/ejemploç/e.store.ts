...
import { Observable, delay, exhaustMap, filter, finalize, of, switchMap, tap, withLatestFrom, catchError } from 'rxjs';
...
export class LabStore extends ComponentStore<LabState> {
  ...
  readonly requestRichness = this.effect<HTMLInputElement>((trigger$) =>
    trigger$.pipe(
      ...
      tap(([_, autoRead]) => !autoRead && this.patchState({ manualReading: true })),
  ...
  getRichness(): Observable<number> {
    ...
    return of(Math.floor(Math.random() * 3) + Math.random() + 14).pipe(delay(1000));
  }

  // UPDATERS
  readonly toggleStateProp = this.updater((state, prop: BooleanKeysOf<LabState>) => ({
    ...state,
    [prop]: !state[prop],
  }));

  readonly updateSampleBarCode = this.updater((state, sampleBarCode: string) => ({
    ...state,
    sampleBarCode,
  }));
  ...
  private showConfirmation(richness: number): Observable<number | null> {
    ...
    return new Observable((obs) => {
      this.confirmationService.confirm({
        ...
        message: `Lectura:&nbsp;&nbsp;<strong>${formatNumber(richness, 'es', '1.1-1')}</strong> %`,
        accept: () => {
          obs.next(richness);
          obs.complete();
        },
